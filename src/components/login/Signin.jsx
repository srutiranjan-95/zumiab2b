...
import { handleSignin, verifyOtp } from "../../service/apiAuth";
...
export default function SignInPage() {
  ...
  const handleSendOtp = async () => {
    ...
    try {
      ...
      // ACCOUNT REJECTED
      else if (message === "Your request has been rejected") {
        setApiError(
      ...
      // EMAIL NOT EXISTS
      else if (message === "Email not exists !!") {
        setApiError(
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      setLoginError(message || "Invalid OTP");
    } finally {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#eef5ff] via-[#f8fbff] to-[#dcecff] flex items-center justify-center p-4">
      <div className="relative w-full max-w-5xl bg-white rounded-[30px] overflow-hidden shadow-[0_20px_60px_rgba(37,99,235,0.15)] border border-blue-100 grid grid-cols-1 md:grid-cols-2">
        ...
        <button
          onClick={() => navigate("/")}
          className="absolute top-5 right-5 z-50 w-9 h-9 rounded-full bg-white/40 backdrop-blur-md border border-blue-100 flex items-center justify-center shadow-lg hover:bg-blue-50 transition-all cursor-pointer"
        >
        ...
        <div className="p-7 md:p-10 flex flex-col justify-center bg-white">
          ...
          <div className="mb-7">
            ...
            <p className="text-[12px] text-gray-500">
              Login to your account using OTP
            </p>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              {/* API ERROR */}
              {apiError && (
                <p className="text-[11px] text-red-500 mt-1">{apiError}</p>
              )}
            </div>
            ...
            {otpSent && (
              <div>
                ...
                {otpError && (
                  <p className="text-[11px] text-red-500 mt-1">{otpError}</p>
                )}
                ...
                <div className="flex items-center justify-between mt-2">
                  ...
                  <button
                    ...
                    className="text-[11px] text-blue-600 hover:text-blue-700 hover:underline flex items-center gap-1 disabled:opacity-70 cursor-pointer"
                  >
                    {loadingResend ? (
                      <>
            ...
            {!otpSent ? (
              <button
                ...
                disabled={loadingOtp}
                className="w-full h-10 rounded-xl bg-blue-600 hover:bg-blue-700 transition-all text-white text-[12px] font-medium flex items-center justify-center gap-2 mt-2 shadow-lg shadow-blue-200 disabled:opacity-70 hover:cursor-pointer"
              >
                {loadingOtp ? (
                  <>
...
/>
          ...
          {/* Floating Card */}
          <div className="absolute bottom-8 left-8 bg-white/40 backdrop-blur-md rounded-2xl px-5 py-4 shadow-xl border border-white/40">
            <p className="text-[11px] text-gray-500 mb-1">Trusted by</p>
